...
class FlightTest < Minitest::Test
  ...
  def test_can_get_info
    ...
    assert_equal "B6", @flight.carrier(0)
    assert_equal "1186", @flight.number(0)
  end

  def test_can_get_origin_and_destination
    assert_equal "RDU", @flight.origin(0)
    assert_equal "JFK", @flight.destination(0)
  end
  ...
  def test_itinerary
    assert_equal [{"leg"=>1, "origin"=>"RDU", "destination"=>"JFK",
      ...
      {"leg"=>2, "origin"=>"JFK", "destination"=>"LAX", "flight_number"=>"B6 2023",
         "departure_time"=>"2016-06-23 at 10:45 PM"}], @flight.itinerary(0)
  end

  def test_departs_at
    assert_equal "07:20 PM", @flight.departs_at(0)
  end
  ...
  def test_options
    assert_equal "hey", @flight.options(3)
  end
